### Tugas Praktikum 03 - Pemograman Web 1
<hr />

<b>Nama : </b>Eko Muchamad Haryono <br />
<b>Kelas : </b>TI - 02 <br />
<b>Prodi : </b>Teknik Informatika <br />
<b>Dealine </b>Tugas : 03 Oktober 2023 ==>> 17 Oktober 2023 <br />
<b>Mata Kuliah : </b>Pemograman Web 1 (PW1) <br />

<hr />

<h3>Penjelasan Tugas </h3><br />

<b>Tugas Utama :</b>
1. Membuat Website <b> Form Pemesanan Barang dan Hasil Pemesanan Barang</b> Menggunakan, HTML dan CSS Inline :
   
   Step Pengerjaan :

   - Buat file form-produk.html harus ada input type : text, number, file, checkbox, submit dan select.
  
   - Buat file hasil-produk.html tampilkan hasilnya di dalam table.

<b>Sub Tugas : </b>

2. Boleh Jika Ingin Melampirkan Tugas <b>Form Pizza</b> di Samping Tugas Utama

3. Boleh Jika Ingin Melampirkan Tugas <b>Form Input Pengguna</b> di Samping Tugas Utama

<hr />
<h3>Informasi Lokasi Tugas : </h3><br />
<b>Tugas Utama : </b>

1. Form Pemesanan Barang dan Hasil Pemesanan Barang = </br>  Directory Repo : <b></b>
      1. Form : <a href="https://github.com/ekomh170/Tugas_Praktikum_Web_PW1/blob/ry_dev/Praktikum_03/form_produk/form-produk.html">Praktikum_03/form_produk/form-produk.html</a>

   
      2. Hasil : <a href="https://github.com/ekomh170/Tugas_Praktikum_Web_PW1/blob/ry_dev/Praktikum_03/form_produk/hasil-produk.html">Praktikum_03/form_produk/hasil-produk.html</a>
   
<b>Sub Tugas : </b>

   1. Sub Tugas = Form Pizza dan Hasil Pizza = Directory Repo :

      1. Form : <a href="https://github.com/ekomh170/Tugas_Praktikum_Web_PW1/blob/ry_dev/Praktikum_03/form_pizza/form-pizza.html">Praktikum_03/form_pizza/form-pizza.html</a>
   
      2. Hasil : <a href="https://github.com/ekomh170/Tugas_Praktikum_Web_PW1/blob/ry_dev/Praktikum_03/form_pizza/hasil-pizza.html">Praktikum_03/form_pizza/hasil-pizza.html</a>

   2. Sub Tugas = Form Input Pengguna dan Hasil Input Pengguna = Directory Repo : 
   
      1. Form : <a href="https://github.com/ekomh170/Tugas_Praktikum_Web_PW1/blob/ry_dev/Praktikum_03/form/form.html">Praktikum_03/form/form.html</a>
   
      2. Hasil : <a href="https://github.com/ekomh170/Tugas_Praktikum_Web_PW1/blob/ry_dev/Praktikum_03/form/hasil.html">Praktikum_03/form/hasil.html</a>

<hr />

<h3>Hasil Preview Tugas : </h3>

Tugas Utama :

1. Form Pemesanan Barang dan Hasil Pemesanan Barang
   
   1. Form : 
      <img src="form_produk/assets/readme/form_produk.png" alt="Form Produk"> 

   2. Hasil : 
      <img src="form_produk/assets/readme/hasil_produk.png" alt="Hasil Produk"> 
      
Sub Tugas :

1. Sub Tugas = Form Pizza dan Hasil Pizza
   
   1. Form :
      <img src="form_pizza/assets/readme/form_pizza.png" alt="Form Pizza">

   2. Hasil :
      <img src="form_pizza/assets/readme/hasil_pizza.png" alt="Hasil Pizza">
 
2. Sub Tugas = Form Input Pengguna dan Hasil Input Pengguna
   
   1. Form :
      <img src="form/assets/readme/form.png" alt="Form">

    2. Hasil :
       <img src="form/assets/readme/hasil.png" alt="Hasil"> 
 

<hr />
<h3>Inspirasi : </h3>

1. IMG :  https://unsplash.com/
2. Favicon  : https://ais.nurulfikri.ac.id/ &  https://elena.nurulfikri.ac.id/
3. ICO : https://www.flaticon.com/free-icons/store
